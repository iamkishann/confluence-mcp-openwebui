---
title: "Configuration"
description: "Configure MCP Atlassian for Open WebUI, Cursor, Windsurf, and VS Code with environment variables"
---
...
|-----|----------|
| Open WebUI | Admin Settings → External Tools → Add Server |
| Cursor | Settings → MCP → + Add new global MCP server |
| Windsurf | MCP settings UI |
| VS Code (Copilot) | MCP configuration in settings or workspace tools UI |

### Open WebUI (Streamable HTTP)

Use this when Open WebUI is running in Docker and MCP Atlassian is exposed over HTTP:
...
If Open WebUI runs in Docker and MCP Atlassian runs on the host, use `http://host.docker.internal:8000/mcp` instead of `localhost`.

### Basic Configuration (stdio clients)
...
### Confluence-Only Open WebUI Service

For a smaller Open WebUI integration surface, restrict the server to Confluence toolsets:
...
| `MCP_CORS_ALLOW_ORIGINS` | Comma-separated CORS origins for HTTP MCP endpoints, or `*` (default). Useful for browser-based connection checks. |

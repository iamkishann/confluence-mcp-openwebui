...
description: "Platform compatibility matrix — Open WebUI, Cursor, Cline, and Cloud vs Server/DC feature comparison"
---
...
|----------|--------|-----------|-------|
| Open WebUI | Fully supported | streamable-http | Best fit for container deployments |
...
Open WebUI supports MCP over Streamable HTTP. Recommended setup notes:

<Steps>
  <Step title="Use Streamable HTTP">
    Start MCP Atlassian with `--transport streamable-http` or `TRANSPORT=streamable-http`, then connect Open WebUI to the `/mcp` endpoint.
  </Step>

  <Step title="Use container-aware URLs">
    If Open WebUI and MCP Atlassian run in the same Docker Compose stack, use `http://mcp-atlassian:8000/mcp`.

    If Open WebUI runs in Docker and MCP Atlassian runs on the host, use `http://host.docker.internal:8000/mcp`.
  </Step>

  <Step title="Prefer scoped toolsets">
    For Confluence-focused deployments, set `TOOLSETS=confluence_pages,confluence_comments,confluence_attachments` and consider `READ_ONLY_MODE=true` until write access is required.
  </Step>
</Steps>

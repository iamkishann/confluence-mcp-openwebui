...
<Tabs>
  ...
  <Tab title="Local Shell">
    ```bash
    MCP_VERY_VERBOSE=true MCP_LOGGING_STDOUT=true uvx mcp-atlassian --transport streamable-http
    ```
  </Tab>
</Tabs>

If you're connecting from an Open WebUI container, also inspect the Open WebUI backend logs:
